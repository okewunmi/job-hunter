...
const STATUSES = [
  ...
  { value: 'email_sent', label: '✉ Auto-Applied' },
    ...
    <div style={{
      ...
      <div style={{ padding: 'clamp(12px, 2vw, 18px)', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, flexWrap: 'wrap' }}>
          ...
          {job.ats_score != null && (
            <div style={{ fontSize: 11, fontWeight: 600, padding: '3px 7px', borderRadius: 6, background: 'rgba(139,92,246,0.1)', color: '#a78bfa' }} title="ATS Score — how well your CV matches the job description">
              ATS {job.ats_score}%
            </div>
          )}
          {job.recruiter_email && (
            <div style={{ fontSize: 11, padding: '3px 7px', borderRadius: 6, background: 'rgba(16,185,129,0.1)', color: '#34d399' }} title={`Recruiter: ${job.recruiter_email}`}>
              📧 Recruiter
            </div>
          )}
      ...
      {expanded && (
        <div style={{ padding: '0 18px 18px', borderTop: '1px solid var(--border)' }}>
          ...
          {(job.recruiter_email || job.ats_score != null) && (
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
              {job.recruiter_email && (
                <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 8, padding: '10px 14px' }}>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>Recruiter Email Found</p>
                  <p style={{ fontSize: 13, color: '#34d399', fontWeight: 600 }}>{job.recruiter_email}</p>
                  {job.recruiter_name && <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>{job.recruiter_name}</p>}
                </div>
              )}
              {job.ats_score != null && (
                <div style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 8, padding: '10px 14px' }}>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 4 }}>ATS Score</p>
                  <p style={{ fontSize: 18, color: '#a78bfa', fontWeight: 800 }}>{job.ats_score}<span style={{ fontSize: 13 }}>%</span></p>
                  <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>CV vs job description match</p>
                </div>
              )}
            </div>
          )}
...
function JobsInner() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/jobs?${params}&t=${Date.now()}`, { cache: 'no-store' });
      const data = await res.json();
      setJobs(data.jobs || []);
